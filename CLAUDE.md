...
liveries:
  ...
  - color: najbrt2
    name_en: "Najbrt 2"
    name_cs: "Najbrt 2"
    # Optional per livery (used to port an upstream object 1:1 into a family):
    # copyright: "TommPa9"   # this livery's upstream credit instead of the family's
    # fields: {cost: 4251600, intro_year: 1993, retire_year: 2009}  # override the
    #                        # vehicles' fields in this livery only
```
...
The ČD loco-hauled coaches (`vehicle-rail/ceske-drahy/coach_*`, `13ev`, `comfortjet`) come from `cdcoach.py` + the driver `cd_coaches.py`, which also writes their `family.yaml` (`python tools/railrender/cd_coaches.py [family …] [--yaml] [--preview DIR]`; review the yaml diff). Same vagonWEB method as `rjcoach.py` (drawings `popisy/img/CD/<name>-a.gif`), 24.5 m cars as length 12, 26.4–26.9 m as 13, in Najbrt 2 (Najbrt 1 = grey roof, no light line; BD = blue lower body; restaurant / sleeper / couchette cars take the dark window band). The yellow 1st-class line covers only the 1st-class part (`first` spans). Look-alike types keep the real details that differ: window rhythm, plug / folding / honecker doors, AC units vs roof vents, pictograms, and on the ex-ÖBB cars the window frame colour (vagonWEB variants `-br` / `-cr` / `-mr` = white / black / blue frames). Driving trailers (sysel, 13Ev) come as `-front` (cab leading) and `-rear` (cab at the tail), each rendered, not `reverse`d. Double-deckers use `dd()`. The `railjet`, `interjet` and `comfortjet` families are 1:1 ports of the CZR sets (art frozen in `src/czr_cd_jets.png` in the CZR pak's object order, lifted 4 px, the pak-extract offset; coupling lists, cost, capacity, speed and weight as read from the pak, table `CZR` in `cd_coaches.py`). The ComfortJet adds the Afmpz 880 (the CZR railjet cab car) and the BRmpz 882 (the ARbmpz 892 without the yellow line).

The ČD locomotives (`vehicle-rail/ceske-drahy/`: 111, 113, 151, 162, 163, 193, 210, 242, 362, 371, 380, 384, 704, 714, 743_2, 750, 750_7, 754, 794, 799, 1216) are rendered the same way, one script per body group, each taking `[family …] [--preview DIR]`: `cd_loco_e99.py` (162/163/362/371 on the approved Eso box), `cd_loco_modern.py` (193/384 on `vectron.py`), `cd_loco_classic.py` (the 1216 Taurus; its `Taurus` class also paints the ÖBB 1116/1216 and PKP EU44 through `taurus.py`), `cd_loco_centrecab.py` (111/113/210), `cd_loco_hood.py` (714, 743.2) and `cd_loco_shunter.py` (704/794/799). Three classes keep TommPa9's hand-drawn pak128.cs sheets because the user preferred them to any render: the 151, 242 and 380 (static sheets, extracted and shifted up 4 px to source coordinates, credited per family). The Brejlovec (754, 750.7, 750) is `tools/railpaint/cd_brejlovec.py`: TommPa9's drawing repainted into each livery, since no box render read as a Brejlovec. All paint comes from `cd_loco_livery.py` (colour slugs and names too): Najbrt 2 on a loco is `LOCO_SKY` upper body, a white stripe at headlight level and sapphire below, lighter than the RAL 5015 on the units, as every loco photo shows.

The Prague metro and the Petřín funicular (`vehicle-rail/dpp/`: 81_71m, m1, lanovka_1985, lanovka_2026 → `VZ-DPPraha-rail.pak`) come from `dpp_metro.py` (`python tools/railrender/dpp_metro.py [family …] [--preview DIR]`). Metro cars (19.2–19.5 m) are length 9, like TommPa9's native Metro_8171M, at coach width (the true 2.71 m section looked skinny next to the natives). Each five-car set is only buildable whole: separate rows for the lead and rear cab cars (length 9, so no `reverse`), and the repeated middle car (4Mt, M1.2) appears twice as `-2` / `-4` on one row. The metro is `electric` like the native one. The funicular cars (length 6) are drawn level, since the engine draws every vehicle level on slopes, and are `battery` vehicles, so the funicular track needs no catenary.
...
Rail signals live under `signal-rail/signals/signal.yaml` (`group: Signals` → `dist/VZ-Signals-rail.pak`). Object `name=` is `VZ-Signals-<id>`; `fields` are the makeobj roadsign keys (`is_signal`, `is_presignal`, `free_route`, `is_longblocksignal`, `single_way`, `end_of_choose`, `min_speed`, `cost`, `offset_left`, `intro_year`, …). The build emits `obj=roadsign`, `waytype=track`, `Image[4*row+col]` for every state row, then `cursor` and `icon`.

Sheet layout (from `tools/sign_extract.py` and `tools/gen_signals.py`): 4 columns, the image directions N, S, W, E, by one row per state, plus a last row with the cursor in col 0 and the 32×32 icon in col 1. The engine only uses states 0 (red) and 1 (green) for plain, choose, long and P signals, and state 2 for presignals. So signals ship 8 images and presignals 12. Never use 16 or more: the engine then reads images 8+ as a second set for electrified track (`signal_t::calc_image`).

Two keys exist only in the simutrans-122.0 fork (PR #4): `is_platformsignal=1` (P, with `is_signal=1`; 8 images) and `station_boundary=1` (LT; exactly 4 images, since more than 4 makes it a traffic light). The stock makeobj silently drops them, so the set says `makeobj: fork` and builds with `MAKEOBJ_FORK_PATH`. Where that variable is unset, the build skips the pak and the install step keeps the installed copy. (`install: false` on a set would build it into `dist/hold/`, which is never installed.)

- **D1**, in three styles: `New` (AŽD 70), `Old` (SSSR mast signals) and `Dwarf` (SSSR dwarfs). Each style has Block, Shunt, PreSignal, Choose, Long, P and LT (`VZ-Signals-D1-<Style>-<Function>`). The style is only the look; the function is shown by a plate (štítek) under the head in the real D1 colours: block white, shunt blue, presignal black, long red, choose red plus a direction indicator box on top of the head, lit when clear, P red plus a white track-number plate. LT replaces the style's head with a numbered lichoběžníková tabulka (D1 čl. 1002).
- **D3**: `VZ-Signals-D3-LT` is the plain lichoběžníková tabulka (D1 čl. 976). `VZ-Signals-D3-P` is the Místo zastavení board (čl. 988) with a red/green lamp on top.
- **1:1 ports**: the one-way and end-of-choose signs, the Minimum speed boards and RailGate, under `VZ-Signals-<original name>`.

The D1/D3 sheets are generated from the extracted pak128.cs art in `signal-rail/pak128cs-source/` (all 76 objects of `infrastructure.rail_signals.pak`, with `objects.yaml`): run `python tools/gen_signals.py signal-rail/signals [--only ID] [--preview DIR]`. To change the art, change the generator. Lamps use the always-lit specials `FF211D`, `01DD01`, `FFFF53` and `0101FF`. `signal-rail/migration-map.tsv` maps every old pak128.cs signal name to its VZ successor, keeping the same function flag, for the savegame migration. The mechanical ČSD semaphores (`Mechanic_*`) are not in the VZ pak. They are kept, with their original names, in `legacy/rail-signals-mechanic/infrastructure.rail_signals_mechanic.pak`, a committed backup built with the stock makeobj from the `.dat` and sprites next to it.
